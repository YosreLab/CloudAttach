...
class Plugin implements PluginInterface
{
    ...
    {
        $secretId = new Text('secretId', null, null, 'Secret ID *', '腾讯云API密钥ID，在访问管理中获取');
        $form->addInput($secretId);

        $secretKey = new Text('secretKey', null, null, 'Secret Key *', '腾讯云API密钥Key');
        ...
        $region = new Select('region', array(
            ...
            'na-siliconvalley' => '硅谷',
            'na-ashburn' => '弗吉尼亚'
        ), 'ap-guangzhou', '存储地域 *', '选择COS存储桶所在地域');
        $form->addInput($region);

        $bucket = new Text('bucket', null, null, '存储桶名称 *', 'COS存储桶名称，格式：bucket-name-appid');
        $form->addInput($bucket);

        $domain = new Text('domain', null, null, 'CDN域名', '自定义CDN加速域名，如：https://cdn.example.com');
        $form->addInput($domain);

        $storagePath = new Text('storagePath', null, 'usr/uploads', '对象存储路径', '自定义COS存储路径，如：usr/uploads 或 attachments');
...
<div id="cos-attachment-manager">
    ...
    <div class="cos-panel-content">
        ...
        <div>
            ...
            <!-- 分类选项卡 -->
            <div id="cos-category-tabs" style="display: flex; gap: 8px; margin-bottom: 15px; flex-wrap: wrap;">
                <button class="cos-category-tab active" data-category="all" style="padding: 6px 12px; border: 1px solid #e0e0e0; background: #1e88e5; color: white; border-radius: 4px; cursor: pointer; font-size: 12px; transition: all 0.3s;">全部</button>
                <button class="cos-category-tab" data-category="image" style="padding: 6px 12px; border: 1px solid #e0e0e0; background: white; color: #666; border-radius: 4px; cursor: pointer; font-size: 12px; transition: all 0.3s;">图片</button>
                <button class="cos-category-tab" data-category="document" style="padding: 6px 12px; border: 1px solid #e0e0e0; background: white; color: #666; border-radius: 4px; cursor: pointer; font-size: 12px; transition: all 0.3s;">文档</button>
                <button class="cos-category-tab" data-category="archive" style="padding: 6px 12px; border: 1px solid #e0e0e0; background: white; color: #666; border-radius: 4px; cursor: pointer; font-size: 12px; transition: all 0.3s;">压缩包</button>
                <button class="cos-category-tab" data-category="other" style="padding: 6px 12px; border: 1px solid #e0e0e0; background: white; color: #666; border-radius: 4px; cursor: pointer; font-size: 12px; transition: all 0.3s;">其他</button>
            </div>
            ...
            <!-- 分页控件 -->
            <div id="cos-pagination" style="display: none; margin-top: 15px; padding-top: 15px; border-top: 1px solid #eee;">
                <div style="display: flex; justify-content: center; align-items: center; gap: 10px; white-space: nowrap;">
                    <button id="cos-prev-page" style="padding: 6px 12px; border: 1px solid #e0e0e0; background: white; color: #666; border-radius: 4px; cursor: pointer; font-size: 12px;">上一页</button>
                    <span id="cos-page-info" style="color: #666; font-size: 12px;">第 1 页 / 共 1 页</span>
                    <button id="cos-next-page" style="padding: 6px 12px; border: 1px solid #e0e0e0; background: white; color: #666; border-radius: 4px; cursor: pointer; font-size: 12px;">下一页</button>
                </div>
            </div>
        </div>
    </div>
...
(function() {
    ...
    window.refreshCosAttachments = function() {
        console.log("刷新附件列表, 分类:", currentCategory, "页码:", currentPage);
        const xhr = new XMLHttpRequest();
        const url = handlerUrl + "?action=list&category=" + currentCategory + "&page=" + currentPage;
        xhr.open("GET", url);
        xhr.onload = function() {
            console.log("列表请求完成, HTTP状态:", xhr.status);
            if (xhr.status === 200) {
                try {
                    const response = JSON.parse(xhr.responseText);
                ...
                console.error("请求失败: HTTP " + xhr.status);
            }
        };
        xhr.onerror = function() {
            console.error("网络错误");
    ...
    function updatePagination(pagination) {
        const paginationDiv = document.getElementById("cos-pagination");
        const pageInfo = document.getElementById("cos-page-info");
        const prevBtn = document.getElementById("cos-prev-page");
        const nextBtn = document.getElementById("cos-next-page");
        ...
        pageInfo.textContent = "第 " + currentPage + " 页 / 共 " + totalPages + " 页 (共 " + pagination.total + " 个文件)";
    ...
    // 分类切换
    document.querySelectorAll(".cos-category-tab").forEach(function(tab) {
        tab.addEventListener("click", function() {
            const category = this.getAttribute("data-category");
            if (category === currentCategory) return;

            // 更新选中状态
            document.querySelectorAll(".cos-category-tab").forEach(function(t) {
    ...
    // 分页按钮
    document.getElementById("cos-prev-page").addEventListener("click", function() {
        if (currentPage > 1) {
            currentPage--;
            refreshCosAttachments();
        }
    ...
    document.getElementById("cos-next-page").addEventListener("click", function() {
